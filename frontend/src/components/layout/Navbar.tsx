  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-200">
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex min-h-[4.5rem] sm:min-h-[5rem] items-center justify-between gap-2 py-2">
          <Link to="/" className="flex items-center gap-2 sm:gap-3 min-w-0 shrink-0" aria-label="XpertIntern Home">
            <img src="/logo.png" alt="" className="h-12 sm:h-14 md:h-16 w-auto object-contain" />
            <span className="rounded bg-brand-accent px-1.5 py-0.5 text-xs font-medium text-white shrink-0">AICTE & UGC</span>
          </Link>
          ...
          <div className="hidden md:flex md:items-center md:gap-4 lg:gap-6 xl:gap-8">
            {navLinks.map(({ to, label }) => (
              <Link
                key={to}
                to={to}
                className={`text-sm font-medium transition ${isActive(to) ? 'text-brand-accent' : 'text-brand-navy hover:text-brand-accent'}`}
              >
                {label}
              </Link>
            ))}
            <div className="ml-2 lg:ml-4 flex items-center gap-2 lg:gap-3">
              <Link
                to="/login"
