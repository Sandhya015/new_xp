...
type CourseRow = { id: string; title: string; category: string; universities: string; mode: string; duration: string; price: number; active: boolean }
...
export function CourseManager() {
  ...
  useEffect(() => {
    let cancelled = false
    adminService.getCourses({ search: search || undefined })
      .then((res) => { if (!cancelled) setItems((res.items || []) as CourseRow[]) })
              ...
              {items.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[row.active ? 'Active' : 'Inactive'] ?? 'bg-gray-100 text-gray-700'}`}>
